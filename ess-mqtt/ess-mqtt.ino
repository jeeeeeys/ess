...
// =============================================================================
// Features:
//  - Smart WiFi manager (NVS, AP portal, double-reset detect)
//  - MQTT telemetry + command channel
//  - RS485: HALFDUPLEX + robust recovery (reinit on write & on repeated read failures)
//  - Manual Modbus read/write via MQTT command messages
//  - Fan relays (R1_1, R1_2, R1_3)
//  - Per-register scaling, batch helper, CRC, etc.
//
// Notes:
//  - Telemetry/Modbus read interval is configurable over MQTT.
...
void ensureOtaStarted() {
  if (!networkServicesInitialized) {
    ArduinoOTA.setHostname("ess-mqtt");
    ArduinoOTA.begin();
...
const unsigned long DEFAULT_TELEMETRY_INTERVAL_SECONDS = 300UL;
...
void buildTopics();
void connectMqtt();
void publishStatus(const char *eventName, const char *result = nullptr, const char *details = nullptr);
void publishTelemetry(float battVoltage, float battCurrent, float soc, float battTemp,
                      float gridVoltage, float gridCurrent, float gridFreq,
                      float invTemp, float inverterVoltage, float inverterCurrent, float inverterFreq);
void mqttCallback(char *topic, byte *payload, unsigned int length);
bool handleCommandJson(const JsonDocument &doc);
...
void setup() {
  ...
  Serial.printf("Startup default telemetry interval = %lu sec (%lu ms)\n", telemetryIntervalSeconds, telemetryIntervalMs);
  snprintf(deviceIdHex, sizeof(deviceIdHex), "%012llX", ESP.getEfuseMac());
  buildTopics();
  Serial.println("MQTT topics:");
  Serial.println(String("  data: ") + topicData);
  Serial.println(String("  status: ") + topicStatus);
  Serial.println(String("  cmd: ") + topicCmd);
  ...
  // -------- Start network services ONLY WHEN CONNECTED --------
  if (WiFi.status() == WL_CONNECTED) {
    ensureOtaStarted();
    initMqttSecurity();
    connectMqtt();
...
void loop() {
  ...
  if (millis() - lastReadTime >= telemetryIntervalMs) {
    ...
    publishTelemetry(battVoltage, battCurrent, soc, battTemp, gridVoltage, gridCurrent, gridFreq,
                     invTemp, inverterVoltage, inverterCurrent, inverterFreq);

    Serial.println("MQTT: values updated.");
    lastReadTime = millis();
  }
}
...
void buildTopics() {
  snprintf(topicData, sizeof(topicData), "ess/update/%s/data", deviceIdHex);
  snprintf(topicStatus, sizeof(topicStatus), "ess/update/%s/status", deviceIdHex);
  snprintf(topicCmd, sizeof(topicCmd), "ess/update/%s/cmd", deviceIdHex);
}

void publishStatus(const char *eventName, const char *result, const char *details) {
...
void publishTelemetry(float battVoltage, float battCurrent, float soc, float battTemp,
  ...
  doc["battCurrent"] = battCurrent;
  doc["soc"] = soc;
  doc["battTemp"] = battTemp;
  doc["gridVoltage"] = gridVoltage;
  doc["gridCurrent"] = gridCurrent;
  doc["gridFreq"] = gridFreq;
  doc["invTemp"] = invTemp;
  doc["inverterVoltage"] = inverterVoltage;
  doc["inverterCurrent"] = inverterCurrent;
  doc["inverterFreq"] = inverterFreq;
  doc["fan1"] = fan1_state;
  doc["fan2"] = fan2_state;
  doc["fan3"] = fan3_state;
...
void connectMqtt() {
  ...
  char clientId[48];
  snprintf(clientId, sizeof(clientId), "ess-%s", deviceIdHex);
  ...
  publishStatus("mqtt_connected", "ok", nullptr);
...
bool handleCommandJson(const JsonDocument &doc) {
  ...
  if (!strcmp(cmd, "set_inverter_run_stop")) {
    int value = doc["value"] | -1;
    if (value < 0 || value > 1) return false;
    sendModbusWriteSingleRegister(slaveID, reg_InverterRunStop, (uint16_t)value);
    publishStatus("command_ack", "ok", "set_inverter_run_stop");
    return true;
  }

  if (!strcmp(cmd, "set_charge_mode")) {
    int value = doc["value"] | -1;
    if (value < 0 || value > 2) return false;
    sendModbusWriteSingleRegister(slaveID, reg_ForceChargeMode, (uint16_t)value);
    publishStatus("command_ack", "ok", "set_charge_mode");
    return true;
  }

  if (!strcmp(cmd, "set_fan_manual")) {
    int value = doc["value"] | -1;
    if (value != 0 && value != 1) return false;
    ...
    publishStatus("command_ack", "ok", "set_fan_manual");
    return true;
  }

  if (!strcmp(cmd, "set_telemetry_interval_sec")) {
    int value = doc["value"] | -1;
    if (!isValidTelemetryInterval(value)) return false;
    telemetryIntervalSeconds = (unsigned long)value;
    telemetryIntervalMs = telemetryIntervalSeconds * 1000UL;
    publishStatus("command_ack", "ok", "set_telemetry_interval_sec");
    return true;
  }

  if (!strcmp(cmd, "modbus_write_single")) {
    int reg = doc["reg"] | -1;
    int value = doc["value"] | -1;
    if (reg < 10000 || reg > 30000 || value < 0 || value > 65535) return false;
    sendModbusWriteSingleRegister(slaveID, (uint16_t)reg, (uint16_t)value);
    publishStatus("command_ack", "ok", "modbus_write_single");
    return true;
  }

  if (!strcmp(cmd, "modbus_read_single")) {
    int reg = doc["reg"] | -1;
    if (reg < 1 || reg > 65535) return false;
    float r = readModbusRegister(slaveID, (uint16_t)reg);
    StaticJsonDocument<320> ack;
    ack["timestamp"] = millis();
    ack["deviceID"] = deviceIdHex;
    ack["event"] = "command_ack";
    ack["command"] = "modbus_read_single";
    ack["reg"] = reg;
    if (r < -1000.0f) {
      ack["result"] = "error";
    } else {
...
void mqttCallback(char *topic, byte *payload, unsigned int length) {
  ...
  if (err) {
    publishStatus("command_ack", "error", "invalid_json");
    return;
  }
  if (!handleCommandJson(doc)) {
    publishStatus("command_ack", "error", "invalid_command_or_value");
  }
}
